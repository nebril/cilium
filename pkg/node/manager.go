...
const (
	// BPFRoute is the route type to set up the BPF tunnel maps
	BPFRoute RouteType = 1 << iota
...
func UpdateNode(ni Identity, n *Node, routesTypes RouteType, ownAddr net.IP) {
	...
	if (routesTypes & L3Route) != 0 {
		updateIPRoute(ni, oldNode, n, ownAddr)
	}
...
func updateIPRoute(ni Identity, oldNode, n *Node, ownAddr net.IP) {
